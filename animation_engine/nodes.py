...
class CompositorNode:
    ...
    def get_input(self, name: str) -> any:
        ...
        if not port:
            return None
        # Si hay conexión, el valor ya fue propagado por el graph
        return port.value
    ...
    def from_dict(cls, d: dict) -> "CompositorNode":
        ...
        # Reconectar inputs/outputs se hace en NodeGraph
        return node
...
class TransformNode(CompositorNode):
    ...
    def process(self, frame: int, ctx: NodeContext):
        ...
        # Motion blur vectorial: múltiples instancias con opacidad decreciente
        if p.get("motion_blur") and self.inputs["in"].connected_to:
            # Samples de motion blur
            samples = max(2, min(32, int(p.get("samples", 8))))
            shutter = p.get("shutter_angle", 180.0) / 360.0  # como fracción de frame
            # Leer transform del frame anterior para calcular delta
            # (simplificado: blur radial basado en rotación)
            blur_parts = []
...
class BlendNode(CompositorNode):
    ...
    def process(self, frame: int, ctx: NodeContext):
        ...
        # En SVG, mezcla se hace vía CSS mix-blend-mode o filter
        # Para simplicidad: blend como capa superpuesta
...
class PreCompNode(CompositorNode):
    ...
    def process(self, frame: int, ctx: NodeContext):
        ...
        # El ctx debería tener un registro de subgraphs
...
class NodeGraph:
    ...
    def _topo_sort(self) -> List[str]:
        ...
        while queue:
            ...
            for m in adj[n]:
                ...
                if in_degree[m] == 0:
                    queue.append(m)
        # Si hay ciclos, ignora nodos no alcanzables
    ...
    def evaluate(self, frame: int, ctx: NodeContext) -> str:
        ...
        for nid in order:
            ...
            if not node.enabled:
                continue
            # Propagar inputs desde conexiones
            for port_name, port in node.inputs.items():
                if port.connected_to:
                    # Tomar el primero (single connection por ahora)
